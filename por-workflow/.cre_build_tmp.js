...
class AbiDecodingDataSizeTooSmallError2 extends BaseError3 {
...
class AbiDecodingZeroDataError2 extends BaseError3 {
  constructor({ cause } = {}) {
...
class AbiEncodingArrayLengthMismatchError2 extends BaseError3 {
...
class AbiEncodingBytesSizeMismatchError2 extends BaseError3 {
...
class AbiEncodingLengthMismatchError2 extends BaseError3 {
...
class AbiFunctionNotFoundError2 extends BaseError3 {
...
class AbiFunctionOutputsNotFoundError2 extends BaseError3 {
  constructor(functionName, { docsPath }) {
    super([
...
class InvalidAbiEncodingTypeError2 extends BaseError3 {
...
class InvalidAbiDecodingTypeError2 extends BaseError3 {
  constructor(type, { docsPath }) {
    super([
...
class InvalidDefinitionTypeError2 extends BaseError3 {
  constructor(type) {
    super([
...
class InvalidAddressError3 extends BaseError3 {
  constructor({ address }) {
    super(`Address "${address}" is invalid.`, {
      metaMessages: [
        "- Address must be a hex value of 20 bytes (40 hex characters).",
...
class IntegerOutOfRangeError3 extends BaseError3 {
...
function prepareEncodeFunctionData2(parameters) {
  ...
  return {
    abi: [abiItem],
    functionName: toFunctionSelector3(formatAbiItem(abiItem))
  };
}
...
var eventSchema = exports_external.object({
  event_name: exports_external.string(),
  kwh_reduced: exports_external.number().gt(KWH_MIN).lte(KWH_MAX),
  kwh_scaled: exports_external.string().regex(/^\d+$/),
  completed_at: exports_external.number()
});
var sumKwhScaled = (raw) => {
  ...
  if (total > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new Error(`total ${total} exceeds MAX_SAFE_INTEGER — migrate consensus to bigint-safe encoding`);
  }
  return { total: Number(total), count };
};
var JouleCreditReserveAbi = [
  {
    type: "function",
...
var configSchema = exports_external.object({
  ...
  reserveAddress: exports_external.string(),
  chainSelectorName: exports_external.string(),
  maxDeltaPerWrite: exports_external.string()
});
var fetchAttestation = (sendRequester, config) => {
...
var reserveEvmClient = (runtime2) => {
  ...
  return new ClientCapability(network286.chainSelector.selector);
};
var readAttestedTotal = (runtime2) => {
  const evm = reserveEvmClient(runtime2);
  const callData = encodeFunctionData2({ abi: JouleCreditReserveAbi, functionName: "attestedKwhTotal" });
  const call = evm.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress2,
      to: runtime2.config.reserveAddress,
      data: callData
    }),
...
var writeAttestation = (runtime2, newTotal) => {
  const evm = reserveEvmClient(runtime2);
  const payload = encodeAbiParameters2([{ type: "uint256" }], [newTotal]);
  const report2 = runtime2.report(prepareReportRequest(payload)).result();
  const resp = evm.writeReport(runtime2, { receiver: runtime2.config.reserveAddress, report: report2 }).result();
  if (resp.txStatus !== TxStatus.SUCCESS) {
    throw new Error(`writeReport failed: ${resp.errorMessage || resp.txStatus}`);
  }
  const txHash = bytesToHex3(resp.txHash || new Uint8Array(32));
  runtime2.log(`Attestation written: total=${newTotal} tx=${txHash}`);
  return txHash;
};
var doAttestation = (runtime2) => {
  const attestation = new ClientCapability2().sendRequest(runtime2, fetchAttestation, ConsensusAggregationByFields({ total: median, count: median }))(runtime2.config).result();
  const newTotal = BigInt(attestation.total);
  runtime2.log(`Consensus attestation: total=${newTotal} count=${attestation.count}`);
  const current = readAttestedTotal(runtime2);
  runtime2.log(`On-chain attestedKwhTotal=${current}`);
  if (newTotal <= current) {
    runtime2.log(`Skip write: newTotal=${newTotal} <= current=${current}`);
    return "skipped";
  }
  const delta = newTotal - current;
  const maxDelta = BigInt(runtime2.config.maxDeltaPerWrite);
  if (delta > maxDelta) {
    throw new Error(`Delta ${delta} exceeds maxDeltaPerWrite ${maxDelta} — refusing to write. ` + `Manual investigation required (likely data corruption or schema change).`);
  }
  return writeAttestation(runtime2, newTotal);
};
var onCronTrigger = (runtime2, payload) => {
  if (!payload.scheduledExecutionTime) {
    throw new Error("Scheduled execution time is required");
  }
  runtime2.log("PoR cron trigger fired");
  return doAttestation(runtime2);
};
